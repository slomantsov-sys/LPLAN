  ...
  return(
    <Overlay>
      <MB style={{maxHeight:"92vh",overflowY:"auto"}}>
            ...
            {bookings.map(b=>{
              ...
              return(
                <div key={b.id} style={{border:`1px solid ${pc}44`,borderRadius:9,padding:"10px 12px",marginBottom:8,background:`${pc}0a`}}>
                  ...
                  <ClientInput value={b.client} onChange={v=>onUpdateBooking(dk,b.id,{client:v})}
                    suggestions={knownClients} placeholder="Клиент"
              ...
              <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                {activePriorities.map(pk=>{
                  ...
                  return <button key={pk} onClick={()=>setAddForm(f=>({...f,priority:pk}))} style={{
                    ...
                    {pp.name}
                    {!hasCapacity&&<span style={{fontSize:7,color:"#f87171",marginLeft:3}}>лимит</span>}
                  </button>;
                })}
              </div>
              {!avPriorities.includes(addForm.priority)&&(
                <div style={{fontSize:9,color:"#f87171",marginTop:4}}>
                  ⚠ Лимит этого приоритета на неделю достигнут — для клиентов недоступно, но ты можешь добавить вручную
                </div>
              )}
            </div>
            {/* Type */}
        ...
        {bookings.length===0&&(
          <Row>
            <Btn onClick={()=>{onToggleOpen();onClose();}} c={isOpen?"#facc15":"#4ade80"} b={isOpen?"#facc15":"#4ade80"} bg="transparent">
